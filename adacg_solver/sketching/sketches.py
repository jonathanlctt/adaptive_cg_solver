...
def hadamard_matrix(n, sketch_size, with_torch=True, device='cpu'):
    order = (np.int64(np.ceil(np.log(n) / np.log(2))))
    if with_torch:
        h = torch.tensor([[1]], device=device, dtype=torch.int32)
        for _ in range(order):
            h = torch.cat([torch.cat([h, h], dim=1), torch.cat([h, -h], dim=1)], dim=0)
        signs = 2 * (torch.rand(1, n, device=device) > 0.5) - 1
    else:
...
def sjlt(a, sketch_size, nnz_per_column=1):
    ...
    if sketch_size == 0:
        return torch.zeros((sketch_size, d), dtype=a.dtype, device=a.device) if with_torch else np.zeros((sketch_size, d), dtype=a.dtype)
    ...
    if not with_torch:
        ...
        data = torch.randn(n, dtype=a.dtype, device=a.device) / np.sqrt(nnz_per_column)
        rows = rng.choice(sketch_size, n, replace=True)
        ccol_indices = torch.arange(n + 1, dtype=torch.int32)
        s = torch.sparse_csc_tensor(ccol_indices, rows, data, device=a.device, dtype=a.dtype, size=(sketch_size, n))
        sa = s @ a
        return sa